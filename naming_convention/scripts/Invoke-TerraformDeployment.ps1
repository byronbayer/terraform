...
.PARAMETER TenantId
    Azure tenant ID (optional, used for display only)

.PARAMETER AutoApprove
    Skip confirmation prompts for apply

.PARAMETER Upgrade
    Pass -upgrade to terraform init
...
function Invoke-TerraformCommand {
    ...
    Write-TerraformLog "Command: terraform $($Arguments -join ' ')" -Level Info
    ...
    try {
        if ($StreamOutput) {
            ...
            $output   = & terraform @Arguments 2>&1 | Out-String
            $exitCode = $LASTEXITCODE
    ...
    try {
        ...
        if ($planData.resource_changes) {
            foreach ($change in $planData.resource_changes) {
                ...
                # Check replace (delete+create) before create to avoid misclassification
                if     ($actions -contains 'delete' -and $actions -contains 'create') { $updates++ }
                elseif ($actions -contains 'create')  { $creates++ }
...
"@
        ...
        if ($planData.resource_changes -and ($creates + $updates + $deletes) -gt 0) {
            ...
            foreach ($change in $planData.resource_changes) {
                ...
                # Check replace before create to avoid misclassification
                if     ($actions -contains 'delete' -and $actions -contains 'create') {
...
function Invoke-TerraformDeployment {
    ...
    try {
        ...
        if (-not $SubscriptionId) {
            ...
            if ($TenantId) { Write-TerraformLog "Tenant ID: $TenantId" -Level Info }
        ...
        # Build variable arguments as an array — safe against injection via & operator splatting
        $varArgs = @($Variables.GetEnumerator() | ForEach-Object { "-var=$($_.Key)=$($_.Value)" })

        # Format + Init
        if ($Action -in @('full', 'init')) {
            try {
                Invoke-TerraformCommand -Arguments @('fmt', '-recursive') -Description "Format Terraform files"
            }
            catch {
            ...
            Invoke-TerraformCommand -Arguments $initArgs -Description "Initialize Terraform"
        }

        # Validate
        if ($Action -in @('full', 'validate', 'plan', 'apply', 'destroy')) {
            Invoke-TerraformCommand -Arguments @('validate') -Description "Validate Terraform configuration"
        }

        # Plan
        if ($Action -in @('full', 'plan', 'apply', 'destroy')) {
            if ($isDestroy) {
                $planArgs = @('plan', '-destroy', "-out=$planFile") + $varArgs
                $planDesc = "Generate destroy plan"
            }
            else {
                $planArgs = @('plan', "-out=$planFile") + $varArgs
                $planDesc = "Generate Terraform plan"
            }
            Invoke-TerraformCommand -Arguments $planArgs -Description $planDesc

            try {
                Write-TerraformLog "Converting plan to JSON..." -Level Info
                $planJson = & terraform show -json "$planFile"
                [System.IO.File]::WriteAllText($planJsonFile, ($planJson -join "`n"))
                ...
                if ($summary) {
                    ...
                    Write-TerraformLog "  - Resources no-op:     $($summary.NoOps)" -Level Info
                }
            }
            catch {
        ...
        # Apply
        if ($Action -in @('full', 'apply', 'destroy')) {
            ...
            $applyDesc = if ($isDestroy) { "Destroy infrastructure" } else { "Apply Terraform plan" }
            Invoke-TerraformCommand -Arguments @('apply', "$planFile") -Description $applyDesc -StreamOutput
...
# Example 6: Initialize Terraform (with provider upgrade)
Invoke-TerraformDeployment -Action init -Upgrade
